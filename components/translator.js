...
const replacer = (wordTranslated, translateStr, replacedStr) => {
  translateStr.forEach((word, i) => {
    ...
    wordTranslated = wordTranslated.replace(
      regex,
      `<span class="highlight">${replacedStr[i]}</span>`
    );
  });
...
const replacerTitle = (wordTranslated, translateStr, replacedStr) => {
  translateStr.forEach((word, i) => {
    const regex = new RegExp(`(?<=^|[.'"\\s])${word}(?=[.'"\\s]|$)`, "gi");
    replacedStr[i] = replacedStr[i].replace(
      replacedStr[i][0],
      replacedStr[i][0].toUpperCase()
    );
    wordTranslated = wordTranslated.replace(
      regex,
      `<span class="highlight">${replacedStr[i]}</span>`
...
const replacerClock = (symTranslated, translateSym, replacedSym) => {
  ...
  return symTranslated.replace(
    regex,
    `<span class="highlight">$1${replacedSym}$2</span>`
  );
};
